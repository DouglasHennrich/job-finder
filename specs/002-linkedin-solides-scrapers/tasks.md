...
- [ ] T001 Update `source` field comment in `scrapers/base.py` to include `"linkedin"` and `"solides"` as valid values (one-line change to the inline comment on the `source: str` field)
...
- [ ] T002 [US1] Implement `scrapers/linkedin.py` with `LinkedInScraper(BaseScraper)`:
...
- [ ] T003 [US1] Write `tests/unit/test_linkedin_scraper.py` smoke test:
...
- [ ] T004 [P] [US2] Implement `scrapers/solides.py` with `SolidesScraper(BaseScraper)`:
...
- [ ] T005 [P] [US2] Write `tests/unit/test_solides_scraper.py` smoke test:
...
- [ ] T006 [US3] Register `LinkedInScraper` and `SolidesScraper` in `main.py`:
