...
- [x] T001 Add `beautifulsoup4>=4.12` to `requirements.txt` (required by US2 CapgeminiScraper)
- [x] T002 Create `pytest.ini` at project root with content: `[pytest]\npythonpath = src` (required to keep tests passing after US1 migration)
...
- [x] T003 Create `src/` directory; move `analyzer.py`, `config.py`, `main.py` from project root into `src/`
...
- [x] T004 [P] [US1] Move `llm/` package directory to `src/llm/` (rename/mv the directory)
- [x] T005 [P] [US1] Move `obsidian/` package directory to `src/obsidian/`
- [x] T006 [P] [US1] Move `resume/` package directory to `src/resume/`
- [x] T007 [P] [US1] Move `scrapers/` package directory to `src/scrapers/`
- [x] T008 [US1] Update `com.douglashennrich.jobfinder.plist`: change `ProgramArguments` second entry from `.../main.py` to `.../src/main.py` (WorkingDirectory stays unchanged)
- [x] T009 [US1] Verify migration: from project root run `python src/main.py 2>&1 | head -5` to confirm no `ModuleNotFoundError`; run `pytest tests/ -v` to confirm all existing tests pass
...
- [x] T011 [P] [US2] Update `source` comment in `src/scrapers/base.py` to add `"capgemini"` to the valid sources list: `# "google_jobs" | "indeed" | "himalayas" | "linkedin" | "solides" | "capgemini"`
- [x] T012 [P] [US2] Create `tests/unit/test_capgemini_scraper.py` with three smoke tests: (1) `fetch()` returns a `list`; (2) graceful failure when `requests.get` raises (mock with `side_effect=RuntimeError`); (3) any returned jobs have `source == "capgemini"`
...
- [x] T013 [US3] Register `CapgeminiScraper` in `src/main.py`: add import `from scrapers.capgemini import CapgeminiScraper`; instantiate and add to the scrapers list alongside existing scrapers; use query from `_build_queries()` (e.g. `"senior fullstack developer nodejs react remote"`)
...
- [x] T014 Run `pip install -r requirements.txt` to install `beautifulsoup4`; verify no dependency conflicts
- [x] T015 Run full test suite `pytest tests/ -v` from project root; confirm 100% of tests pass including new `test_capgemini_scraper.py`
